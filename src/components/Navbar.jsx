...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full px-6 md:px-8 py-4 flex justify-between items-center bg-white dark:bg-darkBg border-b border-gray-200 dark:border-gray-700 transition-all duration-300 z-50">
      <audio ref={audioRef} src="/public/music.mp3" loop preload="auto" />

      <h1 className="text-lg font-semibold text-black dark:text-darkText">Bridge Group Solutions</h1>
      ...
      {/* Desktop & Tablet Menu */}
      <div className="hidden lg:flex items-center space-x-8 text-black dark:text-darkText text-sm font-medium">
      ...
      <button className="lg:hidden p-2 text-black dark:text-white" onClick={() => setMenuOpen(!menuOpen)}>
        {menuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
      </button>

      {menuOpen && (
        <div className="absolute top-16 left-0 w-full md:max-w-md bg-white dark:bg-darkBg shadow-md flex flex-col items-center py-4 space-y-4 lg:hidden">
